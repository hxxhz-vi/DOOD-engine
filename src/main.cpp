...
void
draw_rectangle (std::vector<uint32_t> &img, const size_t x, const size_t y,
...
main (void)
{
  ...
  std::vector<uint32_t> framebuf (win_w * win_h,
              ...
              for (float t = 0; t < 20; t += .05)
                {
                  ...
                  if (map_x < 0 || map_x >= map_w || map_y < 0
                      || map_y >= map_h)
                    break;
                  ...
                  // framebuf[px + py * win_w] = pack_color(255, 255, 255,
                  // 255);
          ...
          for (int y = 0; y < SCREEN_HEIGHT; ++y)
            {
              for (int x = 0; x < SCREEN_WIDTH; ++x)
                {
                  pixels[y * pitch_in_pixels + x]
                      = framebuf[y * SCREEN_WIDTH + x];
                }
            }
